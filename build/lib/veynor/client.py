...
class Client:
    ...
    def pulse(
        ...
        *,
        ...
        """
        Plain-English prediction market briefing synthesized from live data.

        Covers top markets by volume, whale activity, wide spread opportunities,
        price movers (momentum/reversal signals), and volume spikes (unusual flow).
        Synthesis is powered by Claude Haiku on the server — no LLM setup needed.
        ...
        Returns
        -------
        dict with keys: summary (plain-English briefing), highlights (structured
        data by category), meta (venue, category, credits_used, data_age, as_of).
        ...
        >>> pulse = client.pulse()
        >>> print(pulse["summary"])
        'NBA playoffs dominate Kalshi today — Spurs/Wolves G5 leads at $1.5M...'
        """
        return self._get("/v1/pulse", venue=venue, category=category)
